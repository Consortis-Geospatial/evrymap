...
var featureEdit = (function() {
  $(document).ready(function() {
    $('#lnkLogin').show();
    $('#lnkLogin').html(
      "<span class='glyphicon glyphicon-log-in' aria-hidden='true'></span>&nbsp;&nbsp;" + $.i18n._('_LOGINBUTTON')
    );
  ...
  return {
    ...
    showConnected: function(uname, enc) {
      ...
      $('#lnkLogin').html(
        "<span class='glyphicon glyphicon-log-out' aria-hidden='true'></span>&nbsp;" + $.i18n._('_LOGOUT')
      );
      $('#editTools').show();
      $('#usermenu').prepend(
        '<li id="userinfo"><p style="position:relative;top:15px;color:white">' +
          $.i18n._('_USERNAME') +
          ': ' +
          uname +
          '</p></li>'
    ...
    renderEditTools: function() {
      var str =
        ' <div id="editTools" style="display:none" class="righttb btn-group-vertical">' +
    ...
    initAddHole: function() {
      ...
      var currentEditLayerName = $('#hidEditLayer')
        .val()
        .trim();
      ...
      if (currentEditLayer instanceof ol.layer.Vector) {
        ...
        if (typeof currentEditLayer !== 'undefined') {
          ...
          alert(
            'Could not find edit layer for create: ' + currentEditLayerName + ' or ' + currentEditLayerName + '_EDIT'
          );
          return;
        }
      }
    ...
    initAddPart: function(in_geom) {
      ...
      this.drawIntrAct.on('drawend', function(e) {
        ...
        if (typeof multiGeom === 'undefined') {
          mapUtils.showMessage(
            'danger',
            'Layers with geometry type of [' + editLayer.get('edit_geomtype') + '] are not supported',
    ...
    initDrawing: function() {
      ...
      document.addEventListener('keydown', function(e) {
        if (e.which == 27) di.removeLastPoint();
      });
    ...
    initMerge: function() {
      ...
      if (currentEditLayer instanceof ol.layer.Vector) {
        ...
        if (typeof currentEditLayer !== 'undefined') {
          ...
          alert(
            'Could not find edit layer for merge: ' + currentEditLayerName + ' or ' + currentEditLayerName + '_EDIT'
          );
          return;
        }
      }
        ...
        if (interaction instanceof ol.interaction.Select) {
          ...
          $.ajax({
            ...
            error: function(response) {
              ...
              mapUtils.showMessage(
                'danger',
                $.i18n._('_CHECKLOGERROR'),
                $.i18n._('_MERGEDBERROR') + editLayer.get('label')
              );
            },
    ...
    initSplit: function() {
      ...
      if (currentEditLayer instanceof ol.layer.Vector) {
        ...
        if (typeof currentEditLayer !== 'undefined') {
          ...
          alert(
            'Could not find edit layer for split: ' + currentEditLayerName + ' or ' + currentEditLayerName + '_EDIT'
          );
          return;
        }
      }
      if (
        typeof layer2split.get('edit_split_layer') !== 'undefined' &&
        layer2split.get('edit_split_layer').trim() !== ''
      ) {
        $mymap = $('#mapid').data('map');
    ...
    showStartEditForm: function() {
      ...
      $mymap.getLayers().forEach(function(layer, i) {
        if (typeof layer.get('name') !== undefined) {
          ...
          if (
            layer.get('editable') !== undefined &&
            layer.get('editable') === true &&
            layer.get('edit_pk') !== undefined &&
            layer.get('edit_fields') !== undefined
          ) {
            html =
              html +
              '<div class="radio"><label><input type="radio" name="editlayer" id="Radios' +
              c +
              '" value="' +
              curName +
              '">';
    ...
    setEditTools: function() {
      featureEdit.unselectEditTools();
      var lyrNameToEdit = $('#hidEditLayer')
        .val()
        .trim();
      if (lyrNameToEdit !== '') {
        $('#btnCreate')
        ...
        if (
          typeof tmpLyr2Edit.get('edit_geomtype') !== 'undefined' &&
          (tmpLyr2Edit.get('edit_geomtype') === 'LineString' ||
            tmpLyr2Edit.get('edit_geomtype') === 'MultiLineString') &&
          typeof tmpLyr2Edit.get('edit_split_layer') !== 'undefined' &&
          tmpLyr2Edit.get('edit_split_layer').trim() !== ''
        ) {
          $('#btnSplit').show();
          $('#btnSplit')
            .unbind('click')
        ...
        if (
          ...
          (tmpLyr2Edit.get('edit_geomtype') === 'LineString' ||
            tmpLyr2Edit.get('edit_geomtype') === 'MultiLineString') &&
          typeof tmpLyr2Edit.get('edit_merge_url') !== 'undefined'
        ) {
          $('#btnMerge').show();
          $('#btnMerge')
            .unbind('click')
            .bind('click', function() {
              featureEdit.initMerge(lyrNameToEdit);
            });
        }
        if (
          typeof tmpLyr2Edit.get('edit_geomtype') !== 'undefined' &&
          (tmpLyr2Edit.get('edit_geomtype') === 'Polygon' || tmpLyr2Edit.get('edit_geomtype') === 'MultiPolygon')
        ) {
          $('#btnAddHole').show();
          $('#btnAddHole')
            .unbind('click')
        ...
        if (
          typeof tmpLyr2Edit.get('edit_geomtype') !== 'undefined' &&
          (tmpLyr2Edit.get('edit_geomtype') === 'MultiPolygon' ||
            tmpLyr2Edit.get('edit_geomtype') === 'MultiLineString')
        ) {
          $('#btnAddPart').show();
